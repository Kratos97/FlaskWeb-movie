...
app = Flask(__name__)
# 配置键，用于连接数据的数据库，格式mysql://username:password@server:port/db
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:root@127.0.0.1:3306/movie"
# 配置键，如果设置成True，flask_sqlalchemy会追踪对象的修改并发送信号
app.config["SQLALCHEMY_TRACK_MODDFICATIONS"] = True
...
class Movie(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    comments = db.relationship('Comment', backref='movie')  # 评论日志外键关系关联
...
class Admin(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    adminlogs = db.relationship('Adminlog', backref='administrator')  # 管理员登录日志外键关系关联
    oplogs = db.relationship('Oplog', backref='administrator')  # 操作日志外键关系关联
...
class Adminlog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # 编号
    admin_id = db.Column(db.Integer, db.ForeignKey('administrator.id'))  # 所属管理员
    ip = db.Column(db.String(100))  # 登陆ip
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 登陆时间
